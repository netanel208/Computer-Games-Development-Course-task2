...
public class KeyboardMover: MonoBehaviour {
    ...
    void Update() {
        ...
        if(transform.position.x >= max_x_bound)
        {
            transform.position = new Vector3(min_x_bound, transform.position.y, 0);
        }
        else if(transform.position.x <= min_x_bound)
        {
            transform.position = new Vector3(max_x_bound, transform.position.y, 0);
        }
